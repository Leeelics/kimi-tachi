...
async def test_pub_sub():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            await bus.publish("system", Message(
                sender="system",
...
async def test_tracing():
    ...
    try:
        from kimi_tachi.message_bus.tracing import TraceManager, SpanKind
        
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = TraceManager(storage_dir=tmpdir)
            print(f"✅ TraceManager 创建成功")
            
            # 创建 trace
            trace = manager.start_trace(
                name="workflow_execution",
                attributes={"workflow": "feature_implementation"}
            )
            print(f"✅ Trace 创建成功")
