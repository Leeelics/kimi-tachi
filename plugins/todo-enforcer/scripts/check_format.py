...
def check_todo_format(todos: list) -> dict:
    ...
    for i, todo in enumerate(todos):
        ...
        if not status:
            ...
            issues.append(f"Todo {i+1}: Invalid status '{status}' - use: pending, in_progress, completed")
        
        # Check for action verbs
        if title and not any(title.lower().startswith(v) for v in [
            "create", "update", "delete", "fix", "add", "remove", 
            "implement", "test", "review", "analyze", "refactor",
            "write", "document", "check", "verify"
        ]):
            suggestions.append(f"Todo {i+1}: Consider starting with an action verb (e.g., 'Implement', 'Create', 'Fix')")
