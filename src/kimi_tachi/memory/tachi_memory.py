...
class TachiMemory:
    ...
    def _migrate_mnx_to_memnexus(self) -> None:
        """Migrate legacy .mnx/ directory to .memnexus/ if needed."""
        mnx_path = self.project_path / ".mnx"
        memnexus_path = self.project_path / ".memnexus"

        if not mnx_path.exists():
            return

        if not memnexus_path.exists():
            # Simple rename if .memnexus doesn't exist yet
            mnx_path.rename(memnexus_path)
            return

        # Both exist: merge contents (.mnx wins on conflict)
        import shutil

        for item in mnx_path.iterdir():
            dest = memnexus_path / item.name
            if dest.exists():
                if item.is_dir():
                    shutil.rmtree(dest)
                else:
        ...
        # Remove empty .mnx after migration
        shutil.rmtree(mnx_path)

    def _ensure_memnexus_config(self):
        """Ensure .memnexus/ directory and config exist."""
        self._migrate_mnx_to_memnexus()

        memnexus_path = self.project_path / ".memnexus"
        config_path = memnexus_path / "config.yaml"

        # Create .memnexus directory
        memnexus_path.mkdir(parents=True, exist_ok=True)
    ...
    async def _initialize_storage(self):
        ...
        # Ensure .memnexus/ config exists before memnexus init
        self._ensure_memnexus_config()
